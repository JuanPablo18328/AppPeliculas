...
const fetchBusqueda = async (pagina = 1) => {
    ...
    let url;
    if(tipo === 'movie'){
        url = `https://api.themoviedb.org/3/discover/movie?api_key=c93d8d057b8e369244f82429bc493fb9&include_adult=false&include_video=false&language=es-MX&page=${pagina}&region=US&release_date.gte=${añoInicial}&release_date.lte=${añoFinal}&sort_by=popularity.desc&with_genres=${idGenero}`;
    }
    else if(tipo === 'tv'){
        url = `https://api.themoviedb.org/3/discover/tv?api_key=c93d8d057b8e369244f82429bc493fb9&first_air_date.gte=${añoInicial}&first_air_date.lte=${añoFinal}&include_adult=false&include_null_first_air_dates=false&language=es-MX&page=${pagina}&sort_by=popularity.desc&with_genres=${idGenero}`;
    }
...
siguiente = document.getElementById('pagina-siguiente');
